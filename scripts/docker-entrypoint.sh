...
# Purpose: Remove package-level import-map caches before a fresh Docker install.
# Args: None.
# Returns: 0 after ignored caches have been removed or none were present.
clear_import_map_caches() {
  find flatn mocha-es6 lively.* -maxdepth 1 -type f -name .cachedImportMap.json -delete 2>/dev/null || true
}
